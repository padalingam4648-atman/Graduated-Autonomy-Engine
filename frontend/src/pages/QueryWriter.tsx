...
import { Send, Terminal, Loader2, AlertTriangle, ShieldCheck, CheckCircle2, Check, X } from "lucide-react";
      ...
      <AnimatePresence>
        ...
        {result && (
          ...
          >
            ...
            <ApprovalSection result={result} onResolved={() => {
              // Just visual refresh hack for now, in real app we might refetch audit
            }} />
            
            {result.routing_decision === "autonomous" && result.result && (
              <DataResults result={result.result} />
            )}
          </motion.div>
        )}
      </AnimatePresence>
    ...
    <div className="glass-card">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="p-4 rounded-lg bg-[var(--bg-secondary)] border border-[var(--border-light)]">
          ...
          <div className="font-medium truncate">{Object.keys(risk_score.breakdown)[0] || "Query"}</div>
        </div>
      </div>
    </div>
  );
...
function ApprovalSection({ result, onResolved }: { result: ProposeResponse, onResolved: () => void }) {
  ...
  const handleResolve = async (decision: "approve" | "reject" | "confirm") => {
    ...
    try {
      if (result.routing_decision === "confirm") {
        ...
        setResolvedStatus(res.execution_status === "success" ? "Executed" : "Failed");
      } else if (result.routing_decision === "full_review") {
        await resolveReview((result as any).review_id, decision as any, "admin");
        setResolvedStatus(decision === "approve" ? "Approved & Executed" : "Rejected");
      }
      onResolved();
    } catch (err) {
      console.error(err);
  ...
  if (!result.rows || result.rows.length === 0) {
    return (
      <div className="glass-card">
        ...
        <p className="text-sm text-[var(--text-secondary)]">{result.detail}</p>
      </div>
    );
  }
  ...
  return (
    <div className="glass-card flex flex-col gap-4">
      <div className="flex justify-between items-center">
        <h3 className="font-bold">Database Results</h3>
